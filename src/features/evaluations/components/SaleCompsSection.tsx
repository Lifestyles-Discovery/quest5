...
export default function SaleCompsSection({
  ...
  const immediateSearch = (inputs: Partial<SaleCompInputs>) => {
    cancelSearch();
  ...
  const sortedComps = useMemo(() => {
    if (!sortKey) return saleComps;
    return [...saleComps].sort((a, b) => {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      {/* Comps List */}
      <div className={`overflow-x-auto transition-opacity duration-150 ${updateSaleComps.isPending ? 'opacity-60' : ''}`}>
        {saleComps.length === 0 ? (
